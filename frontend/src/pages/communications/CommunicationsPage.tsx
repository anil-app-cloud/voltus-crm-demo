...
const CommunicationsPage = () => {
  ...
  useEffect(() => {
    performFetchCommunications();
  }, []);
  // This is the actual fetch implementation
  const performFetchCommunications = async () => {
    console.debug('Performing communications fetch operation');
    
    // Cancel any previous requests
    if (abortControllerRef.current) {
      console.debug('Canceling previous fetch request');
    ...
    try {
      console.debug('Starting API calls with abort signal');
      // Pass the signal to API calls for cancellation support
      const [commsData, customersData] = await Promise.all([
        getCommunications({ signal }),
        getAllCustomers({ signal })
      ]);
      
      // Create an ID for this fetch operation to track in logs
      console.log("Fetching communications", commsData);
      const fetchId = Date.now();
      console.debug(`Fetch operation ${fetchId} completed successfully`);
      ...
      // Always set loading to false unless the request was aborted
      if (!signal.aborted) {
        setIsLoading(false);
      }
    } catch (error) {
      // Only handle non-cancellation errors
      const isCanceled = isCancelError(error) || signal.aborted;
      console.error("Error fetching communications:", error);
      if (!isCanceled) {
        // Only log and show toast for real errors, not cancellations
       
        showToast({ message: "Failed to fetch communications", type: "error" });
      } else {
        // For debugging purposes only - remove in production
        console.debug("Request was canceled:", error);
      }
      
      // Reset loading state for non-cancellation errors
      if (!isCanceled) {
        setIsLoading(false);
      }
    }
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {(!currentCommunication.tags || currentCommunication.tags.length === 0) && (
                    <span className="text-sm text-gray-500">No tags added</span>
                  )}
                </div>
              </div>
              
              {(currentCommunication.type === 'email' || currentCommunication.type === 'note') && currentCommunication.content && (
                <div className="mb-6">
                  <h3 className="text-sm font-medium mb-2">{currentCommunication.type === 'email' ? 'Email Content' : 'Note Content'}</h3>
                  <div className="bg-white border p-4 rounded-md whitespace-pre-line">
                    {currentCommunication.content}
                  </div>
                </div>
              )}
              
              {currentCommunication.type === 'call' && currentCommunication.summary && (
                <div className="mb-6">
                  <h3 className="text-sm font-medium mb-2">Call Summary</h3>
                  <div className="bg-white border p-4 rounded-md">
                    {currentCommunication.summary}
                  </div>
                </div>
              )}
              ...
              <div className="border-t pt-4 flex justify-end space-x-3">
                <Button 
                  variant="outline" 
                  onClick={() => {
                    // Mark communication as having follow-up
                    if (currentCommunication.tags && !currentCommunication.tags.includes('follow-up')) {
                      handleAddTag(currentCommunication.id, 'follow-up');
                    }
                    
                    setIsDetailViewOpen(false);
                  }}
                >
                  <CheckCircle2 className="mr-2 h-4 w-4" /> Mark for Follow-up
                </Button>
                ...
                <Button onClick={() => {
                  setIsDetailViewOpen(false); 
                  handleEditCommunication(currentCommunication);
                }}>
